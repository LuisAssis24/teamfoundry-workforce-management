...
import com.teamfoundry.backend.account.service.EmployeeRegistrationService;
 ...
 */
@RestController
@RequestMapping(value = "/api/employee/verification", produces = MediaType.APPLICATION_JSON_VALUE)
...
public class EmployeeVerificationController {
    ...
    public ResponseEntity<GenericResponse> resend(@Valid @RequestBody VerificationResendRequest request) {
        return ResponseEntity.ok(employeeRegistrationService.resendVerificationCode(request));
    }
}
